...
mod tests {
    ...
    fn test_default_config() {
        ...
        // 验证传感器标定配置存在
        assert_eq!(config.sensor_calibration.weight_scale_value, 50.0);
        assert_eq!(config.sensor_calibration.angle_scale_value, 90.0);
        assert_eq!(config.sensor_calibration.radius_scale_value, 20.0);
        
        // 验证额定载荷表存在
        assert!(!config.rated_load_table.entries.is_empty());
        assert_eq!(config.rated_load_table.entries.len(), 8);
    }
    ...
    fn test_validate_invalid_sensor_calibration() {
        ...
        // 设置无效的传感器标定参数（除零错误）
        config.sensor_calibration.weight_scale_ad = config.sensor_calibration.weight_zero_ad;
    ...
    fn test_validate_invalid_load_table() {
        ...
        // 设置无效的载荷表（空表）
        config.rated_load_table.entries.clear();
    ...
    fn test_validate_invalid_angle_thresholds() {
        ...
        // 设置无效的角度阈值（报警值小于预警值）
        config.sensor_calibration.angle_alarm_value = 70.0;
        config.sensor_calibration.angle_warning_value = 75.0;
        
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("angle_alarm_value"));
    }
    ...
    fn test_validate_invalid_moment_thresholds() {
        ...
        // 设置无效的力矩阈值（报警值小于预警值）
        config.sensor_calibration.moment_alarm_percentage = 80.0;
        config.sensor_calibration.moment_warning_percentage = 90.0;
        
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("moment_alarm_percentage"));
    }
    ...
    fn test_load_table_access() {
        ...
        // 测试可以访问额定载荷表
        let rated_load = config.rated_load_table.get_rated_load(5.0);
        assert_eq!(rated_load, 40.0);
        
        let rated_load = config.rated_load_table.get_rated_load(10.0);
    ...
    fn test_warning_and_alarm_checks() {
        ...
        // 测试角度预警和报警
        assert!(!config.sensor_calibration.is_angle_warning(60.0));
        assert!(config.sensor_calibration.is_angle_warning(75.0));
        assert!(config.sensor_calibration.is_angle_alarm(85.0));
        
        // 测试力矩预警和报警
        assert!(!config.sensor_calibration.is_moment_warning(80.0));
        assert!(config.sensor_calibration.is_moment_warning(90.0));
        assert!(config.sensor_calibration.is_moment_alarm(100.0));
    ...
    fn test_clone() {
        ...
        // 验证克隆的值相同
        assert_eq!(
            config.sensor_calibration.weight_scale_value,
            cloned.sensor_calibration.weight_scale_value
        );
        assert_eq!(
            config.rated_load_table.entries.len(),
            cloned.rated_load_table.entries.len()
        );
    }
}
