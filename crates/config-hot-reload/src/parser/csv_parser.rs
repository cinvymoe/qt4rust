...
use qt_rust_demo::models::rated_load_table::{RatedLoadTable, RatedLoadEntry};
...
fn parse_csv_content(content: &str, path: &Path) -> Result<RatedLoadTable, HotReloadError> {
    ...
    for line in content.lines() {
        ...
        if line.starts_with("moment_warning_threshold,") {
            moment_warning_threshold = parse_threshold_line(line, "moment_warning_threshold", path, line_number)?;
            continue;
        }
        
        if line.starts_with("moment_alarm_threshold,") {
            moment_alarm_threshold = parse_threshold_line(line, "moment_alarm_threshold", path, line_number)?;
            continue;
        }
...
fn parse_threshold_line(
    ...
    if parts.len() != 2 {
        return Err(HotReloadError::ParseError {
            path: path.to_path_buf(),
            reason: format!("第 {} 行: {} 配置格式错误，应为 '{},<value>'", line_number, key, key),
        });
    }
    ...
    let value = parts[1].trim().parse::<f64>().map_err(|_| {
        HotReloadError::ParseError {
            path: path.to_path_buf(),
            reason: format!("第 {} 行: {} 值无法解析为数字: '{}'", line_number, key, parts[1]),
        }
    })?;
...
fn parse_data_line(
    ...
    let boom_length = parts[0].trim().parse::<f64>().map_err(|_| {
        HotReloadError::ParseError {
            path: path.to_path_buf(),
            reason: format!("第 {} 行: 臂长值无法解析为数字: '{}'", line_number, parts[0]),
        }
    })?;
    
    let working_radius = parts[1].trim().parse::<f64>().map_err(|_| {
        HotReloadError::ParseError {
            path: path.to_path_buf(),
            reason: format!("第 {} 行: 工作幅度值无法解析为数字: '{}'", line_number, parts[1]),
        }
    })?;
    
    let rated_load = parts[2].trim().parse::<f64>().map_err(|_| {
        HotReloadError::ParseError {
            path: path.to_path_buf(),
            reason: format!("第 {} 行: 额定载荷值无法解析为数字: '{}'", line_number, parts[2]),
        }
    })?;
