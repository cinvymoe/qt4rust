...
fn main() {
    ...
    collector.start(move || {
        ...
        if let Ok(mut buf) = buffer_clone.write() {
            ...
            // 每 10 次采集打印一次
            if count % 10 == 0 {
                println!("[采集 #{}] 载荷: {:.2}t, 半径: {:.2}m, 缓冲区: {} 条",
                         count, data.load, data.radius, buf.get_count());
            }
        }
    });
    ...
    reader_timer.start(move || {
        if let Ok(buf) = buffer_clone.read() {
            if let Some(latest) = buf.get_latest() {
                println!("[UI 更新] 最新数据: 载荷={:.2}t, 半径={:.2}m, 历史记录={} 条",
                         latest.load, latest.radius, buf.get_count());
            }
        }
    });
