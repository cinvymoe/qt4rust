...
impl Application {
    ...
    fn setup_qml_context(&mut self) -> Result<(), ApplicationError> {
        tracing::info!("Setting up QML context...");
    ...
    pub fn run(&mut self) -> i32 {
        ...
        if let Some(app) = self.qt_app.as_mut() {
            ...
            tracing::info!("Application exiting with code: {}", exit_code);
            exit_code
        } else {
            tracing::error!("Qt application is not initialized");
            1 // Qt 应用初始化失败
        }
    }
}
