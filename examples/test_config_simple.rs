...
fn main() {
    ...
    println!(
        "  采集间隔: {:?}",
        Duration::from_millis(config.collection.interval_ms)
    );
    println!(
        "  存储间隔: {:?}",
        Duration::from_millis(config.storage.interval_ms)
    );
    println!(
        "  重试延迟: {:?}",
    ...
    println!(
        "  统计间隔: {:?}",
        Duration::from_secs(config.monitoring.stats_interval_sec)
    );
