...
async fn main() {
    ...
    for _i in 1..=3 {
        sequence += 1;
        let sensor_data = SensorData::from_tuple(10.0, 10.0, 60.0, false, false); // 低载荷
    ...
    sequence += 1;
    let sensor_data = SensorData::from_tuple(23.0, 10.0, 60.0, false, false); // 高载荷
    simulate_collection_cycle(&pipeline, sensor_data, sequence, &mut last_was_danger).await;
    ...
    for i in 5..=7 {
        sequence += 1;
        let sensor_data = SensorData::from_tuple(24.0 + (i as f64 * 0.1), 10.0, 60.0, false, false);
        simulate_collection_cycle(&pipeline, sensor_data, sequence, &mut last_was_danger).await;
        sleep(Duration::from_millis(50)).await;
    }
    println!();
    ...
    sequence += 1;
    let sensor_data = SensorData::from_tuple(12.0, 10.0, 60.0, false, false); // 载荷下降
    simulate_collection_cycle(&pipeline, sensor_data, sequence, &mut last_was_danger).await;
    sleep(Duration::from_millis(100)).await;
    println!();
    ...
    for _ in 9..=10 {
        sequence += 1;
        let sensor_data = SensorData::from_tuple(8.0, 10.0, 60.0, false, false);
        simulate_collection_cycle(&pipeline, sensor_data, sequence, &mut last_was_danger).await;
        sleep(Duration::from_millis(50)).await;
    }
    println!();
    ...
    sequence += 1;
    let sensor_data = SensorData::from_tuple(25.0, 10.0, 60.0, false, false);
    simulate_collection_cycle(&pipeline, sensor_data, sequence, &mut last_was_danger).await;
    sleep(Duration::from_millis(100)).await;
    println!();
    ...
    println!("周期 12: 载荷下降");
    sequence += 1;
    let sensor_data = SensorData::from_tuple(10.0, 10.0, 60.0, false, false);
    simulate_collection_cycle(&pipeline, sensor_data, sequence, &mut last_was_danger).await;
    sleep(Duration::from_millis(100)).await;
    println!();
